...
# Version:        v11.0 "Guardian" (2025-08-30)
#
# Description:    自律型AIプロジェクトエクスポートツール - 最終版
#                 - v10.1の全機能（自己学習圧縮率等）を継承。
#                 - [NEW] プロジェクト・ガーディアンモード (--watch): Gitリポジトリを
#                   常時監視し、コミットを検知して最新の分析パッケージを
#                   自動的にバックグラウンドで生成する自律動作モードを追加。
#
# 操作するソフト:
...
# 使用方法 (CLI):
#   # 通常実行
#   python tools/code_export_for_ai.py --profile gemini-chronicle
#
#   # ガーディアンモードで起動 (自律監視)
#   python tools/code_export_for_ai.py --watch
#
# 使用方法 (UI):
...
BASE_COMPRESSION_RATIOS = defaultdict(lambda: 0.5, {
    ".py": 0.30, ".md": 0.35, ".txt": 0.40, ".json": 0.25, ".toml": 0.35,
    ".yaml": 0.40, ".yml": 0.40, ".html": 0.20, ".css": 0.25, ".js": 0.28,
    ".ts": 0.28, ".sh": 0.45,
})
...
COMMON_EXCLUDE_DIRS = [
    ...
    "**/env/**", "**/openenv/**", "**/site-packages/**", "**/exports/**", "**/logs/**",
    "**/.mypy_cache/**", "**/.pytest_cache/**", "**/.idea/**", "**/.vscode/**",
    "**/build/**", "**/dist/**", "**/*.egg-info/**", "**/typings/**",
]
    ...
    if COMPRESSION_STATS_FILE.exists():
        try:
            ...
            for ext, data in stats.items():
                if data.get("total_raw_bytes", 0) > 1024:
...
def select_files(items: List[FileItem], target_mb: float, max_single_mb: float, compression_ratios: defaultdict) -> Tuple[List[FileItem], List[Tuple[float, Path]]]:
    picked, heavy = [], []; total_predicted_zip_size = 0.0; target_bytes = target_mb * 1024 * 1024
    for item in items:
        if (item.size_bytes / 1024**2) > max_single_mb:
            heavy.append((item.size_bytes / 1024**2, item.rel_path)); continue
        ratio = compression_ratios[item.path.suffix.lower()]; predicted_zip_bytes = item.size_bytes * ratio
        if total_predicted_zip_size + predicted_zip_bytes > target_bytes:
            heavy.append((item.size_bytes / 1024**2, item.rel_path)); continue
        picked.append(item); total_predicted_zip_size += predicted_zip_bytes
    return picked, heavy

# ============================================================================
# 5. 出力ファイル生成 & Perplexityアップロード (v10.1から変更なし)
# ============================================================================
def create_combined_code_and_info(items: List[FileItem], log: LogSink, profile_name: str) -> Tuple[str, str]:
...
def perplexity_upload(items_to_upload: List[Path], api_token: str, log: LogSink):
...
def main():
    ...
    if args.ui:
        ...
        export_main(args.root, args.profile, args.emit_zip, args.emit_folder, args.dry_run, DEFAULT_EXPORTS_DIR, DEFAULT_LOGS_DIR, Event(), args.api_token)
