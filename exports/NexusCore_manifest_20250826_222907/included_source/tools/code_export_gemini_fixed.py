...
def export_for_gemini(roots: List[Path], profile_name: str,
    ...
    all_scored: List[Tuple[float, Path, Path]] = []
    for r in roots:
    ...
    if PROFILES[profile_name]["mode"] == "manifest":
        ...
        with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            ...
            # === Phase 1: Primary (.py first for Gemini) ===
            if profile_key == "gemini_10mb":
                py_scored = [(s, r, p) for (s, r, p) in all_scored if p.suffix.lower() == ".py"]
                for _, r, p in py_scored:
            ...
            # === Phase 2: Secondary fill ===
            if buf.tell() < floor_bytes:
                # 2-1) 残っている .py を小さい順で優先補充（Gemini）
                if profile_key == "gemini_10mb":
                    remaining_py: List[Tuple[Path, Path]] = []
                    for r in roots:
                        patterns = load_gitignore(r)
                        for p in r.rglob("*.py"):
                            if p.is_file() and not path_is_ignored(p, r, export_root_dir, profile_key, patterns):
                                if p not in seen:
                                    remaining_py.append((r, p))
                    remaining_py.sort(key=lambda x: x[1].stat().st_size)
                    for r, p in remaining_py:
                        if len(picked) >= MANIFEST_INCLUDED_FILES_MAX or buf.tell() >= floor_bytes:
                            break
                        try_add(r, p)

                # 2-2) その他テキスト系（小さい順）
                secondary_cand: List[Tuple[Path, Path]] = []
                for r in roots:
                    patterns = load_gitignore(r)
                ...
                # .py をさらに優先しつつ、同拡張子内は小さい順
                def _ext_rank(path: Path) -> int:
                    return 0 if path.suffix.lower() == ".py" else 1
                secondary_cand.sort(key=lambda x: (_ext_rank(x[1]), x[1].stat().st_size))
...
def create_interface():
    ...
    with gr.Blocks(title="Code Export for Gemini (All-Py First)", theme=gr.themes.Soft()) as demo:
        gr.Markdown("# 🤖 Gemini対応コードエクスポート（.py最優先 / Manifest両生成 / サイズプロファイル）")
