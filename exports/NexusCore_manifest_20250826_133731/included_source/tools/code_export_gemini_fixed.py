...
# --- 更新履歴 ---
# 2025-08-26 rev6.5:
#  - ZIPサイズ急減問題に対処する高度な充填ロジックを正式導入。
#    - 主要コード追加後、最低保証サイズ（例: 9.0MB）に満たない場合、
#      二次候補（ドキュメント、設定等）を小さい順に充填。
#    - それでも不足する場合、主要コードの索引ファイルでパディングし、
#      目標サイズ（例: 9.8MB）を安定して達成する。
#  - 過去のZIPが消える問題に対処するため、出力保持ポリシーを再導入。
#
# 2025-08-26 rev6.4:
#  - NameErrorを修正し、タイムスタンプを同期。
# ======================================================================
...
def load_gitignore(root: Path) -> Set[str]:
    ...
    return {line.strip() for line in gi.read_text("utf-8", "ignore").splitlines() if line.strip() and not line.strip().startswith("#")}.union(IGNORED_FILES_USER)
...
def path_is_ignored(path: Path, root: Path, export_root: Path, profile_key: str, user_patterns: Set[str]) -> bool:
    ...
    return any(fnmatch.fnmatchcase(path_str.casefold(), pat.casefold()) or fnmatch.fnmatchcase(rel_path.name.casefold(), pat.casefold()) for pat in user_patterns)
...
def file_score(path: Path, root: Path) -> float:
    ...
    except (ValueError, OSError): score = base
    return score

def _is_binary(path: Path) -> bool:
    try:
        with open(path, "rb") as f: return b"\0" in f.read(1024)
    except Exception: return True

def _copy_to_manifest_folder(export_root_dir: Path, project_prefix: str, run_id: str, roots: list[Path], picked: list[Tuple[Path, Path]], chronicle_md: str, final_manifest: str, readme_md: str) -> Path:
    ...
    if chronicle_md: (manifest_dir / "PROJECT_CHRONICLE.md").write_text(chronicle_md, encoding="utf-8")
    (manifest_dir / "CODE_STRUCTURE_MANIFEST.md").write_text(final_manifest, encoding="utf-8")
    included_dir = manifest_dir / "included_source"; included_dir.mkdir(exist_ok=True)
...
# メタデータ生成
# ======================================================================
def create_readme_md(project_prefix: str) -> str:
...
def export_for_gemini(roots: List[Path], profile_name: str,
    ...
    if PROFILES[profile_name]["mode"] == "manifest":
        ...
        with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            readme_md = create_readme_md(project_prefix)
