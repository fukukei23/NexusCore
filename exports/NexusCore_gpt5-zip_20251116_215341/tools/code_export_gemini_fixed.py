...
# Version: all-py-first v7.3.3 (FULL)
# Date: 2025-08-29
...
#  - NEW(v7.3.3): Exclude current manifest/exports/logs from input candidates (static + dynamic).
#  - NEW(2025-08-29): Minor hardening, docstrings, and defaults for max-files=5000 kept.
...
PROFILES = {
    "gemini-10": {
        ...
        "exclude_globs": [
            ...
            "**/*.pdf", "**/*.pptx", "**/*.docx",
            "**/node_modules/**", "**/parser.c", "**/tree_sitter_languages/**",
            # NEW: exclude exports/logs/manifests anywhere
            "**/exports/**", "**/logs/**", "**/NexusCore_manifest_*/**",
...
def _hash(s: str) -> str:
    ...
    return _h.sha1(s.encode("utf-8", errors="ignore")).hexdigest()[:10]
...
def shorten_deep_relpath(rel: Path, max_len: int = 200) -> Path:
    ...
    if len(parts) <= 3:
        base = parts[-1]
        stem = base.split(".")[0]
        base_short = f"{stem[:16]}-{_hash(s)}" + (("." + base.split(".", 1)[1]) if "." in base else "")
        return Path("/".join(parts[:-1] + [base_short]))
    head = "/".join(parts[:2])
    tail = parts[-1]
    mid = "/".join(parts[2:-1])
    new_s = f"{head}/__{_hash(mid)}__/__flat__/{tail}"
    return Path(new_s)
...
def should_exclude(p: Path, exclude_globs: List[str], dynamic_prefixes: List[str]) -> bool:
    s = _path_str(p)
    # dynamic absolute-prefix exclusion (current exports / current manifest)
    ...
    for pat in exclude_globs:
        try:
            if PurePath(s).match(pat):
                return True
        except Exception:
            continue
    return False

def list_candidates(roots: List[str], profile: dict, max_files: int,
                    dynamic_prefixes: List[str]) -> List[Path]:
    exts = set(profile["include_exts"]) | set(profile.get("include_configs", set()))
    excl = list(profile["exclude_globs"])
    files: List[Path] = []
    seen = set()
    ...
    for r in roots:
        ...
        for p in root.rglob("*"):
            ...
            if p.suffix.lower() not in exts:
                continue
            if should_exclude(p, excl, dynamic_prefixes):
                continue
            key = str(p.resolve())
            if key in seen:
                continue
    ...
    # sort by (priority, -mtime, name)
    def _key(pp: Path):
        prio = EXT_PRIORITY.get(pp.suffix.lower(), 99)
        try:
            mtime = -pp.stat().st_mtime
        except Exception:
            mtime = 0
        return (prio, mtime, str(pp).lower())
...
def compute_rel(p: Path, roots: List[str]) -> Path:
    for r in roots:
        try:
            return p.resolve().relative_to(Path(r).resolve())
        except Exception:
            continue
    return Path(p.name)
...
def write_manifest_readme(manifest_dir: Path, args, picked_map: List[Tuple[Path, Path]],
    ...
    lines.append("3. Web UI: `python tools/nexus_export_ui.py`.")
    lines.append("4. CLI examples:")
...
def export_main(args):
    ...
    # prepare paths first (so we can dynamically exclude them)
    ...
    # dynamic exclusion prefixes (absolute)
    dynamic_prefixes = [
        str(exports_dir).replace("\\", "/").lower(),      # any existing exports under roots
        str(manifest_dir).replace("\\", "/").lower(),     # the manifest we are creating now
    ]
    ...
    # build progress log
    lines = []
    header = f"{ICON['preview'] if args.dry_run else ICON['pick']}Pick Result: est ZIP {pick.est_total_mb} MB / {len(pick.picked)} files"
    lines.append(header)
    acc = 0.0
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--roots", nargs="*", default=[str(Path('.').resolve())],
                    help="Root directories to include")
