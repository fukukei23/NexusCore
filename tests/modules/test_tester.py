...
from nexuscore.archive.modules.tester import RESULT_LOG, SAMPLE_FILE, SANDBOX_DIR, TEST_FILE, save_and_test_code
...
class TestSaveAndTestCode:
    @patch("nexuscore.archive.modules.tester.subprocess.run")
    def test_returns_stdout_on_success(self, mock_run, tmp_path, monkeypatch):
        monkeypatch.setattr("nexuscore.archive.modules.tester.SANDBOX_DIR", str(tmp_path))
        monkeypatch.setattr("nexuscore.archive.modules.tester.SAMPLE_FILE", str(tmp_path / "sample.py"))
        monkeypatch.setattr("nexuscore.archive.modules.tester.TEST_FILE", str(tmp_path / "test_sample.py"))
        monkeypatch.setattr("nexuscore.archive.modules.tester.RESULT_LOG", str(tmp_path / "result.log"))
    ...
    def test_creates_sample_file_with_given_code(self, mock_run, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("nexuscore.archive.modules.tester.SANDBOX_DIR", str(tmp_path))
        monkeypatch.setattr("nexuscore.archive.modules.tester.SAMPLE_FILE", sample)
        monkeypatch.setattr("nexuscore.archive.modules.tester.TEST_FILE", str(tmp_path / "test_sample.py"))
        monkeypatch.setattr("nexuscore.archive.modules.tester.RESULT_LOG", str(tmp_path / "result.log"))
    ...
    def test_auto_creates_test_file_when_missing(self, mock_run, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("nexuscore.archive.modules.tester.SANDBOX_DIR", str(tmp_path))
        monkeypatch.setattr("nexuscore.archive.modules.tester.SAMPLE_FILE", str(tmp_path / "sample.py"))
        monkeypatch.setattr("nexuscore.archive.modules.tester.TEST_FILE", test_file)
        monkeypatch.setattr("nexuscore.archive.modules.tester.RESULT_LOG", str(tmp_path / "result.log"))
    ...
    def test_writes_combined_output_to_result_log(self, mock_run, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("nexuscore.archive.modules.tester.SAMPLE_FILE", str(tmp_path / "sample.py"))
        monkeypatch.setattr("nexuscore.archive.modules.tester.TEST_FILE", str(tmp_path / "test_sample.py"))
        monkeypatch.setattr("nexuscore.archive.modules.tester.RESULT_LOG", log_file)
    ...
    @patch(
        "nexuscore.archive.modules.tester.subprocess.run",
