...
def test_clone_or_update_repo_with_base_dir(mock_subprocess, tmp_path):
    """clone_or_update_repoがNEXUS_REPO_BASE_DIRからコピーするテスト"""
...
def test_clone_or_update_repo_with_git_clone(mock_subprocess, tmp_path):
    """clone_or_update_repoがgit cloneを実行するテスト"""
...
def test_run_tests_success(tmp_path, monkeypatch):
    ...
    ok, output = run_tests(project_path)
...
def test_get_changed_files_with_git_diff(mock_subprocess, tmp_path):
    """get_changed_filesがgit diffを実行するテスト"""
...
def test_get_changed_files_with_base_and_head(mock_subprocess, tmp_path):
    """get_changed_filesがbase_refとhead_refを使用するテスト"""
    mock_subprocess.run.return_value = MagicMock(
        returncode=0,
        stdout="src/module.py\n",
        stderr="",
...
def test_get_changed_files_handles_git_error(mock_subprocess, tmp_path):
    """get_changed_filesがgitエラーを処理するテスト"""
    mock_subprocess.run.return_value = MagicMock(
        returncode=1,
        stdout="",
...
# ---- patch_workflow (collect_relevant_files) ----


def test_collect_relevant_files_from_stacktrace(tmp_path):
    """collect_relevant_filesがスタックトレースからファイルを収集するテスト"""
    project_path = tmp_path / "test_project"
    project_path.mkdir()
...
def test_collect_relevant_files_from_changed_files(tmp_path):
    """collect_relevant_filesが変更ファイルから収集するテスト"""
    project_path = tmp_path / "test_project"
    project_path.mkdir()
...
def test_collect_relevant_files_fallback_to_py_files(tmp_path):
    """collect_relevant_filesがフォールバックで.pyファイルを拾うテスト"""
    project_path = tmp_path / "test_project"
    project_path.mkdir()
...
# ---- patch_workflow (generate_patch_via_debugger) ----


def test_generate_patch_via_debugger_with_debug_and_patch(tmp_path):
    """generate_patch_via_debuggerがdebug_and_patchメソッドを優先的に呼ぶテスト"""
    debugger_agent = MagicMock()
...
def test_generate_patch_via_debugger_with_method(tmp_path):
    """generate_patch_via_debuggerがgenerate_patchメソッドを呼ぶテスト（フォールバック）"""
    debugger_agent = MagicMock()
...
def test_generate_patch_via_debugger_without_method(tmp_path):
    """generate_patch_via_debuggerがメソッドがない場合のテスト"""
    debugger_agent = MagicMock()
...
def test_generate_patch_via_debugger_exception_handling(tmp_path):
    """generate_patch_via_debuggerが例外を処理するテスト"""
    debugger_agent = MagicMock()
...
# ---- SelfHealingService.run_for_pull_request (integration) ----
...
def test_run_for_pull_request_no_issues(tmp_path):
    ...
    with patch("nexuscore.services.self_healing_service.clone_or_update_repo"):
        with patch("nexuscore.services.self_healing_service.run_tests", return_value=(True, "All tests passed")):
...
def test_run_for_pull_request_no_debugger_agent(tmp_path):
    ...
    with patch("nexuscore.services.self_healing_service.clone_or_update_repo"):
        with patch("nexuscore.services.self_healing_service.run_tests", return_value=(False, "Test failed")):
            result = service.run_for_pull_request(
                repo_full_name="owner/repo",
                pr_number=123,
                head_sha="abc123",
...
def test_run_for_pull_request_patch_generation_failed(tmp_path):
    """パッチ生成に失敗した場合のテスト"""
    debugger_agent = MagicMock()
    debugger_agent.generate_patch = MagicMock(return_value={})
