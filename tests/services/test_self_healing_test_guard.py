...
"""
    ...
    with mock.patch.object(service, "_clone_or_update_repo"):
        with mock.patch.object(service, "_run_tests", return_value=(False, "Test failed")):
...
"""
    ...
    # テストファイルを変更していないので、ブロックされない
    # （実際の適用結果に依存するが、少なくともテストガードでブロックされない）
