...
def test_extract_code_and_reason_handles_json_and_fence():
    code, reason = revision_tab.extract_code_and_reason(json.dumps({"code": "print('hi')", "reason": "ok"}))
...
def test_generate_prompt_includes_sections(tmp_path, monkeypatch):
    ...
    prompt = revision_tab.generate_prompt(str(sample), str(testf), "summary", "hist", "errs", "note")
...
def test_run_pytest_exception(monkeypatch):
    monkeypatch.setattr(revision_tab.subprocess, "run", lambda *a, **k: (_ for _ in ()).throw(RuntimeError("boom")))
    ok, output = revision_tab.run_pytest()
    assert ok is False
