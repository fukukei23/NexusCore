...
def test_llm_disabled_uses_template_only(sample_project_dir, tmp_path, monkeypatch):
    ...
    with patch(
        "nexuscore.utils.test_generator._try_generate_tests_with_llm", side_effect=mock_llm_call
    ):
...
def test_template_mode_handles_parse_error_gracefully(tmp_path):
    ...
    assert (
        result is not None
    ), "generate_template_tests should return a string even for broken files"
    assert isinstance(result, str), f"Expected string, got {type(result)}"

    # "Failed to parse" または "parse error" を含むこと
    assert (
        "Failed to parse" in result or "parse error" in result or "parse_error" in result
    ), f"Expected 'Failed to parse' or 'parse error' in result, got:\n{result}"
...
def test_template_mode_handles_read_error_gracefully(tmp_path):
    ...
    assert (
        result is not None
    ), "generate_template_tests should return a string even for non-existent files"
    assert isinstance(result, str), f"Expected string, got {type(result)}"

    # "Failed to read" または "read error" を含むこと
    assert (
        "Failed to read" in result or "read error" in result or "read_error" in result
    ), f"Expected 'Failed to read' or 'read error' in result, got:\n{result}"
