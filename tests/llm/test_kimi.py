...
# --- テスト対象のモジュールをインポート ---
try:
    from nexuscore.llm.llm_router import BaseLLM, LLMRouter, MoonshotLLM
...
def run_kimi_test():
    ...
    try:
        ...
        if "Kimi基本接続成功" in response:
            logging.info(
                f"✅ [Part 1] テスト成功！ モデル '{client.model_name}' からの応答: {response}"
            )
        else:
            logging.warning(
                f"⚠️ [Part 1] テストは成功しましたが、予期しない応答です。応答: {response}"
            )
    except ValueError as ve:
        ...
        logging.error(
            f"❌ [Part 1] テスト失敗: API呼び出し中にエラーが発生しました。 -> {e}", exc_info=True
        )
        return
    finally:
    ...
    try:
        ...
        if (
            isinstance(client_short, MoonshotLLM)
            and client_short.model_name == expected_short_model
        ):
            logging.info(
                f"✅ [Case 2a] モデル選択成功！期待通り '{expected_short_model}' が選択されました。"
            )
            response_short = client_short.execute(
                prompt=short_prompt, system_prompt="簡潔に日本語で回答せよ。"
            )
            logging.info(f"✅ [Case 2a] APIコール成功！応答: {response_short[:100]}...")
        else:
            logging.error("❌ [Case 2a] モデル選択失敗！")
            logging.error(f"  期待したモデル: {expected_short_model}")
            # ★★★ 修正点: ログ出力のバグを修正 ★★★
            actual_model = (
                client_short.model_name
                if isinstance(client_short, BaseLLM)
                else type(client_short).__name__
            )
    ...
    try:
        ...
        if isinstance(client_long, MoonshotLLM) and client_long.model_name == expected_long_model:
            logging.info(
                f"✅ [Case 2b] モデル選択成功！期待通り '{expected_long_model}' が選択されました。"
            )
            prompt = "この長文テキストの要点を3つにまとめてください。"
            response_long = client_long.execute(
                prompt=prompt, system_prompt="長文要約アシスタントです。日本語で回答してください。"
            )
            logging.info(f"✅ [Case 2b] APIコール成功！応答: {response_long[:200]}...")

        else:
            logging.error("❌ [Case 2b] モデル選択失敗！")
            logging.error(f"  期待したモデル: {expected_long_model}")
            # ★★★ 修正点: ログ出力のバグを修正 ★★★
            actual_model = (
                client_long.model_name
                if isinstance(client_long, BaseLLM)
                else type(client_long).__name__
            )
            logging.error(f"  実際に選択されたモデル: {actual_model}")
