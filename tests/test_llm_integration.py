...
# --- テスト対象を読み込み ---
from nexuscore.llm.llm_router import LLMRouter, GeminiLLM, OpenAILLM, LocalLLM
...
@pytest.mark.parametrize("task_description, router_llm_response, expected_worker_llm_class, expected_worker_model", [
    ("新しい主力商品の革新的な名前を10個考えて", "creative", OpenAILLM, "gpt-4o"),
    ("この関数の複雑なバグを修正して", "analytical", GeminiLLM, "gemini-1.5-pro-latest"),
    ("この顧客データは個人情報保護法に違反していないかレビューして", "secure", LocalLLM, "llama3-local-8b"),
    ("日本の首都はどこですか？", "general", GeminiLLM, "gemini-1.5-flash-latest"),
])
def test_router_end_to_end_decision_making(task_description, router_llm_response, expected_worker_llm_class, expected_worker_model, mocker):
    """
    【最重要テスト】
    ...
    assert called_kwargs.get('prompt') == task_description
    assert called_kwargs.get('system_prompt') == DummyAgent.SYSTEM_PROMPT
