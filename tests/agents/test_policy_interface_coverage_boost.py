...
import nexuscore.agents.policy_interface as pi_module
...
class TestLaunchGradioExceptionPrint:
    """行156-157: launch_gradio内の例外ハンドリングprint文"""

    @patch.object(pi_module, "GRADIO_AVAILABLE", True)
    def test_launch_thread_exception_prints_message(self, capsys):
        """Gradioのlaunch()が例外を投げたとき、print文が実行されることを検証"""
        mock_gr = MagicMock()
        ...
        assert "Gradio起動エラー" in captured.out


class TestFinallyBlockCloseException:
    """行181-185: finally内のinterface.close()例外ハンドリング"""

    @patch.object(pi_module, "GRADIO_AVAILABLE", True)
    @patch.object(pi_module, "gr", MagicMock())
    def test_close_exception_in_finally_prints_message(self, capsys):
        """finallyでself.interface.close()が例外を投げた場合のprint文を検証"""
        pi = PolicyInterface()
        ...
        captured = capsys.readouterr()
        assert "Gradioを閉じる際にエラーが発生" in captured.out

    @patch.object(pi_module, "GRADIO_AVAILABLE", True)
    @patch.object(pi_module, "gr", MagicMock())
    def test_close_success_in_finally_no_error_print(self, capsys):
        """finallyでself.interface.close()が成功した場合のprint文を検証"""
        pi = PolicyInterface()
        ...
        captured = capsys.readouterr()
        assert "Gradioインターフェースを閉じました" in captured.out
