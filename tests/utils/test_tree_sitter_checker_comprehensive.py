...
class TestCheckAvailability:
    ...
    def test_check_availability_success(self):
        ...
        with patch('nexuscore.utils.tree_sitter_checker.TREE_SITTER_AVAILABLE', True), \
             patch.dict('sys.modules', {'tree_sitter_language_pack': MagicMock(get_parser=mock_get_parser)}), \
...
class TestSetupParsers:
    ...
    def test_setup_parsers_success(self):
        ...
        with patch('nexuscore.utils.tree_sitter_checker.TREE_SITTER_AVAILABLE', True), \
             patch('nexuscore.utils.tree_sitter_checker.get_parser', mock_get_parser_func, create=True), \
             patch('nexuscore.utils.tree_sitter_checker.get_language', mock_get_language, create=True):
    ...
    def test_setup_parsers_partial_failure(self):
        ...
        with patch('nexuscore.utils.tree_sitter_checker.TREE_SITTER_AVAILABLE', True), \
             patch('nexuscore.utils.tree_sitter_checker.get_parser', side_effect=get_parser_side_effect, create=True), \
             patch('nexuscore.utils.tree_sitter_checker.get_language', mock_get_language, create=True):
