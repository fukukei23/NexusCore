...
class SlackNotifier:
    ...
    def __init__(self, webhook_url: Optional[str] = None) -> None:
        ...
        if not HAS_REQUESTS:
            ...
            logger.info(
                "NEXUS_SLACK_WEBHOOK_URLが設定されていません。Slack通知は無効になります。"
            )
    ...
    def notify_orchestrator_complete(
        ...
        *,
        ...
        notify_status = "success" if status == "success" else ("error" if status == "error" else "warning")
...
def get_notifier() -> Optional[SlackNotifier]:
    """
