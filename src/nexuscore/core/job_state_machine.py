...
class FailedState(State):
    ...
    def __init__(self, machine: JobStateMachine, error_message: str | None = None):
    ...
    def handle(self) -> None:
        ...
        self.machine._update_state_metadata(
            {"status": "failed", "message": "Job execution failed", "error": self.error_message}
        )
...
class JobStateMachine:
    ...
    def transition_to(self, new_state_class: type[State], **kwargs) -> None:
        ...
        logger.info(f"Job {self.job_id}: State transition {old_state_name} -> {new_state_name}")
    ...
    def fail(self, error_message: str, details: dict[str, Any] | None = None) -> None:
    ...
    def _record_completion(self) -> None:
        ...
        record = RunRecord(
            run_id=self.job_id,
            session_id=(
                self.session_controller.session_id if self.session_controller else self.job_id
            ),
            kind=self.job_type,
            status="success",
    ...
    def _log_state_transition(self, old_state: str, new_state: str) -> None:
        """状態遷移をログに記録"""
        logger.debug(f"Job {self.job_id}: State transition logged: {old_state} -> {new_state}")
