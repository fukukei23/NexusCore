...
def load_api_key() -> tuple[str | None, str]:
    ...
    if api_key:
        logger.info(
            f"    ✅ 環境変数からAPIキーを発見しました (キーの末尾: ...{api_key[-4:]})。これを使用します。"
        )
    ...
    try:
        ...
        if os.path.exists(dotenv_path):
            ...
            if api_key:
                logger.info(
                    f"    ✅ .env ファイルからAPIキーを発見しました (キーの末尾: ...{api_key[-4:]})。これを使用します。"
                )
    ...
    try:
        secrets_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../config/secrets.py")
        )
        if os.path.exists(secrets_path):
            ...
            if api_key:
                logger.info(
                    f"    ✅ secrets.py からAPIキーを発見しました (キーの末尾: ...{api_key[-4:]})。これを使用します。"
                )
    ...
    try:
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "あなたは優秀なPythonプログラマーです。Context Agentの推奨事項に従い、高品質なコードを生成してください。",
                },
...
"""
    ...
    if "from your_module import" in result:
        ...
        result = result.replace(
            "from your_module import", "# from your_module import  # Context Agent により削除"
        )
    progress(1.0, desc="生成完了")
...
def create_streamlit_migrated_tab():
    ...
    with gr.Blocks(
        title="🤖 コード生成 & テストタブ（Context Agent統合版）", theme=gr.themes.Soft()
    ) as interface:
                ...
                with gr.Row():
                    with gr.Column(scale=2):
                        ...
                        generate_btn = gr.Button(
                            "🔧 コード生成 (非同期)", variant="primary", size="lg"
                        )
...
if __name__ == "__main__":
    ...
    if not OPENAI_API_KEY:
        logger.critical(
            "APIキーが設定されていません。環境変数、.env、またはsecrets.pyを確認してください。"
        )

    app = tab_streamlit_port()

    logger.info(
        "Gradioアプリケーションを起動します... [http://127.0.0.1:7860](http://127.0.0.1:7860)"
    )
    app.launch()
