...
def template_finance(name="enterprise_finance") -> Dict[str, Any]:
    ...
    t["rules"] = [
        ...
        {"id": "ACCOUNT", "desc": "銀行口座/カード番号の取扱い禁止または必須マスキング", "severity": "high"},
        {"id": "ADVICE", "desc": "特定銘柄の推奨/断定的助言の禁止。一般的情報提供に留める", "severity": "medium"},
        {"id": "INSIDER", "desc": "未公開情報・内部情報の共有禁止", "severity": "high"},
    ]
    t["guidance"]["disclaimer"] = "本出力は一般的情報であり、投資助言ではありません。最終判断は自己責任です。"
    return t
...
def template_healthcare(name="enterprise_healthcare") -> Dict[str, Any]:
    ...
    t["rules"] = [
        ...
        {"id": "DIAGNOSIS", "desc": "診断の断定・処方指示の禁止。一般的注意喚起のみ", "severity": "high"},
        {"id": "EMERGENCY", "desc": "緊急時は直ちに医療機関へ誘導", "severity": "medium"},
        {"id": "SENSITIVE", "desc": "健康・遺伝情報等の取り扱い注意", "severity": "high"},
    ]
    t["guidance"]["disclaimer"] = "本出力は医療アドバイスではありません。緊急時は医療機関へ連絡してください。"
    return t
...
def _mock_evaluate(rules: Dict[str, Any], text: str) -> Dict[str, Any]:
    ...
    if re.search(r"(診断|処方|薬を飲み|この薬|確実に治る)", text):
        violations.append({"rule_id": "DIAGNOSIS", "severity": "high", "evidence": "医療助言の断定"})
    # 金融
    ...
    with gr.Blocks(title="Enterprise Policy Console", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                gr.Markdown("### プロファイル")
                profile_list = gr.Dropdown(choices=list_profiles(), label="既存プロファイル", value=None)
                new_name = gr.Textbox(label="新規/名前を付けて保存", placeholder="my_enterprise_policy")
                tmpl = gr.Radio(choices=["finance", "healthcare", "education", "general"],
                                value="general", label="テンプレート")
                ...
                gr.Markdown("### ルールJSON")
                policy_json = gr.Code(label="policy.json", language="json", lines=22, value=json.dumps(template_general(), ensure_ascii=False, indent=2))

            with gr.Column(scale=1):
                gr.Markdown("### テストベッド")
                test_input = gr.Textbox(label="評価したいテキスト", lines=8, placeholder="ここに社内想定の入力やモデル出力を貼る")
                btn_eval = gr.Button("このポリシーで評価")
                status_out = gr.Markdown("—")
                violations = gr.Dataframe(headers=["RuleID", "Severity", "Evidence"], datatype=["str", "str", "str"], row_count=(0, "dynamic"))
        ...
        def _save(name: str, text: str):
            ...
            return (gr.update(value=text), msg, gr.update(choices=list_profiles()), gr.update(value=name)) if ok \
                else (gr.update(), msg, gr.update(), gr.update())

        def _delete(name: Optional[str]):
            if not name:
        ...
        # --- イベント配線 ---
        btn_new_from_tmpl.click(_new_from_template, inputs=[new_name, tmpl],
        ...
        btn_save.click(_save, inputs=[new_name, policy_json],
                       outputs=[policy_json, profile_msg, profile_list, profile_list])
        btn_delete.click(_delete, inputs=[profile_list], outputs=[profile_msg, profile_list])

        btn_eval.click(_evaluate, inputs=[policy_json, test_input],
                       outputs=[status_out, violations, redacted])
