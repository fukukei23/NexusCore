...
class SelfHealingService:
    ...
    def run_for_pull_request(
        ...
        *,
        ...
        try:
            ...
            # 9. 自動PR生成（status == "fixed" の場合）
            auto_pr_result = None
            if status == "fixed":
                auto_pr_result = self._auto_create_fix_pr(
                    repo_full_name=repo_full_name,
                    head_sha=head_sha,
                    pr_number=pr_number,
                    run_id=run_id,
                ...
                if auto_pr_result:
                    details["auto_pr"] = auto_pr_result
                    summary += f" Auto-fix PR #{auto_pr_result['pr_number']} created."
    ...
    def _auto_create_fix_pr(
        ...
        *,
        ...
        """
        修正パッチが適用されてテストが通った場合、自動的にPRを作成する。

        Returns:
            成功時: {"pr_number": int, "pr_url": str, "branch": str}
            失敗/スキップ時: None
        """
        github_token = os.getenv("NEXUS_GITHUB_TOKEN") or os.getenv("GITHUB_TOKEN")
        if not github_token:
            self.logger.info(
                "Skipping auto PR creation: NEXUS_GITHUB_TOKEN / GITHUB_TOKEN not set"
            )
            return None

        if not changed_files:
            self.logger.info("Skipping auto PR creation: no changed files")
            return None

        try:
            from nexuscore.integration.github_pr_client import GitHubPRClient

            client = GitHubPRClient(token=github_token, repo_full_name=repo_full_name)

            # ブランチ名: fix/auto-{run_id}
            short_id = run_id.split("-")[1] if "-" in run_id else run_id[:8]
            branch_name = f"fix/auto-{short_id}"

            # PR タイトル・本文
            pr_title = f"🤖 Auto-fix (Run {run_id})"
            pr_body = (
                f"## Self-Healing Auto-Fix\n\n"
                f"Original PR: #{pr_number}\n"
                f"Run ID: `{run_id}`\n"
                f"Base SHA: `{head_sha[:7]}`\n\n"
                f"### Changed Files\n"
                + "\n".join(f"- `{f}`" for f in changed_files)
                + "\n\n---\n*Auto-generated by NexusCore Self-Healing*"
            )

            # パッチをファイルマップに変換
            # 注意: 実際のファイル内容は sandbox 内の project_path から取得する必要があるが、
            # この時点では project_path はスコープ外。代わりにパッチテキストをコミットメッセージに含め、
            # コミットメッセージにパッチ内容を記録する。
            # 実ファイル内容は呼び出し元から渡すべきだが、現在のアーキテクチャでは
            # sandbox 内のファイル内容を直接取得できないため、
            # 空コミット + パッチ内容をコミットメッセージに添付する設計とする。
            # TODO: sandbox内のファイル内容を直接渡す機能の追加

            # 変更ファイルのリストをコミットメッセージに含める
            files_list = "\n".join(f"  - {f}" for f in changed_files)
            commit_message = (
                f"auto-fix: apply self-healing patch (Run {run_id})\n\n"
                f"Changed files:\n{files_list}\n\n"
                f"Patch:\n{patch_text[:2000]}"
            )

            # 空ファイルマップ（パッチ内容はコミットメッセージに記録）
            # 実際のファイル内容は後続のステップで反映する
            result = client.auto_create_fix_pr(
                ...
                pr_title=pr_title,
                pr_body=pr_body,
                files={},  # 空コミット（パッチ情報はメッセージに記録）
            )

            self.logger.info(
                f"Auto-fix PR created: #{result['pr_number']} ({result['pr_url']})"
            )
            return result

        except Exception as e:
            self.logger.warning(f"Auto PR creation failed: {e}", exc_info=True)
            return None
