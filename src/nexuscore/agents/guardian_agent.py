...
from nexuscore.utils.code_analyzer import analyze_code_quality, QualityReport
from nexuscore.agents.mutation_tester_agent import MutationTesterAgent, MutationReport
...
"""
    ...
    def _run_quality_gates(
        ...
        """
        Tier 1とTier 2の品質ゲートを実行する。

        Args:
            source_path: ソースコードのパス
            test_path: テストコードのパス
            project_root: プロジェクトルート（省略時はカレントディレクトリ）
            constitution: 憲法（省略時は自動ロード）

        Returns:
            {
                "tier1": QualityReport,
                "tier2": MutationReport,
                "overall_passed": bool,
                "violations": List[str]
            }
        """
        ...
        try:
            ...
            self.logger.error(f"Tier 1 quality gate failed: {e}", exc_info=True)
            tier1_report = None
        ...
        if tier2_report and not tier2_report.passed:
            overall_passed = False
            violations.append(f"Tier 2不合格: ミューテーションスコア {tier2_report.mutation_score:.1f}% (最低基準: {constitution.get('quality_gates', {}).get('tier2', {}).get('mutation_score_min', 80)}%)")
        ...
        Returns:
            {
                ...
                "reason": str,
                "quality_gates": {...},
                "feedback_for_coder": str (REJECT時)
            }
        """
        self._budget("guardian:review_with_quality_gates")

        # Tier 1とTier 2の品質ゲートを実行
    ...
    def review_and_commit(
        ...
        *,
        ...
        Args:
            ...
            test_result: テスト結果
            testimony: 開発者の証言
            constitution: 憲法（文字列形式）
            task_description: タスク説明
            changed_files: 変更ファイルリスト
            debug_info: デバッグ情報（自己修復時）
            allow_commit: コミット許可フラグ
            branch_name: ブランチ名
            enable_quality_gates: 品質ゲートを有効化するか（デフォルト: False）
            source_path: ソースコードのパス（品質ゲート有効時に必須）
            test_path: テストコードのパス（品質ゲート有効時に必須）
            project_root: プロジェクトルート（省略時はカレントディレクトリ）

        Returns:
            レビュー結果（decision, reason, quality_gates, commit など）
        """
        # 品質ゲートが有効な場合
        if enable_quality_gates:
            ...
            # 憲法を辞書形式に変換
            try:
                constitution_dict = json.loads(constitution) if isinstance(constitution, str) else constitution
            except json.JSONDecodeError:
