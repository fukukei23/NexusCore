...
"""
    ...
    def _run_quality_gates(
        ...
        if tier2_report and not tier2_report.passed:
            overall_passed = False
            violations.append(
                f"Tier 2不合格: ミューテーションスコア {tier2_report.mutation_score:.1f}% (最低基準: {constitution.get('quality_gates', {}).get('tier2', {}).get('mutation_score_min', 80)}%)"
            )
...
"""
    ...
    def _format_quality_gates_summary(self, quality_gates_result: dict[str, Any]) -> str:
        ...
        if tier1:
            ...
            summary += (
                f"- Pylint: {tier1.pylint_score:.1f}/10 {'✅' if tier1.pylint_passed else '❌'}\n"
            )
    ...
    def _generate_commit_message(
        self, review_data: dict, changed_files: list, debug_info: dict = None
    ) -> str:
        scope = "auto"
