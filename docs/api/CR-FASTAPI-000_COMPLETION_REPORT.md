...
- `docs/api/api_inventory.md` に Markdown テーブルとして一覧化
...
**実施内容**:
- `docs/api/api_inventory.md` を新規作成
- 見出し構成:
...
**作成ファイル**:
- `docs/api/api_inventory.md` - API 棚卸し結果ドキュメント

## 変更ファイル一覧

### 新規作成ファイル
- `docs/api/api_inventory.md` - API Inventory ドキュメント（Public / Internal / Deprecated の分類と一覧）
...
**作成されたドキュメント**: `docs/api/api_inventory.md`
...
- [API Inventory](./api_inventory.md) - 本 CR の成果物
- [FastAPI Migration Prompts](./README.md) - FastAPI 移行の全体像
- [.cursorrules](../../.cursorrules) - プロジェクトルール

## まとめ

CR-FASTAPI-000 の実装により、NexusCore リポジトリ内の既存 HTTP API の棚卸しが完了しました。`docs/api/api_inventory.md` に Public / Internal / Deprecated の分類で一覧化され、FastAPI 移行の優先順位を決定するためのベースラインが確立されました。既存コードの挙動には影響を与えず、ドキュメント追加のみで完了しています。
