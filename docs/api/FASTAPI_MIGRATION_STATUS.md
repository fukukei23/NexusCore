...
| Projects   | `/api/v1/projects` (GET)         | `/api/v1/projects` (GET)           | Migrated      | FastAPI 側が正式版。Flask は廃止候補     |
| Projects   | `/api/v1/projects` (POST)        | `/api/v1/projects` (POST)          | Migrated      | FastAPI 側が正式版。Flask は廃止候補     |
| Projects   | `/api/v1/projects/<id>` (GET)    | `/api/v1/projects/{id}` (GET)      | Migrated      | FastAPI 側が正式版。Flask は廃止候補     |
| Runs       | `/api/v1/runs` (GET)             | `/api/v1/runs` (GET)               | Migrated      | FastAPI 側が正式版。Flask は廃止候補     |
| Runs       | `/api/v1/runs/<id>` (GET)         | `/api/v1/runs/{id}` (GET)          | Migrated      | FastAPI 側が正式版。Flask は廃止候補     |
...
| Projects   | `/api/v1/projects/<id>/run` (POST)| `/api/v1/projects/{id}/run` (POST) | Migrated      | FastAPI 側が正式版。Flask は廃止候補     |
| Runs       | `/api/v1/projects/<id>/runs/latest` (GET) | `/api/v1/projects/{id}/runs/latest` (GET) | Migrated      | FastAPI 側が正式版。Flask は廃止候補     |
| Badges     | `/api/projects/<id>/badge/success_rate` (GET) | `/api/projects/{id}/badge/success_rate` (GET) | Migrated      | FastAPI 側が正式版。Flask は廃止候補     |
| Badges     | `/api/projects/<id>/badge/last_run` (GET)    | `/api/projects/{id}/badge/last_run` (GET) | Migrated      | FastAPI 側が正式版。Flask は廃止候補     |
...
以下のエンドポイントは FastAPI に移行済みのため、非推奨（Deprecated）としてマークされています：

- `/api/v1/projects` (GET) - FastAPI `/api/v1/projects` に移行済み

以下のエンドポイントは FastAPI に移行済みのため、非推奨（Deprecated）としてマークされています：

- `/api/v1/projects/<project_id>/run` (POST) - FastAPI `/api/v1/projects/{id}/run` に移行済み（CR-FASTAPI-009）
- `/api/v1/projects/<project_id>/runs/latest` (GET) - FastAPI `/api/v1/projects/{id}/runs/latest` に移行済み（CR-FASTAPI-009）

### `src/nexuscore/webapp/api_badges.py`

以下のエンドポイントは FastAPI に移行済みのため、非推奨（Deprecated）としてマークされています：

- `/api/projects/<project_id>/badge/success_rate` (GET) - FastAPI `/api/projects/{id}/badge/success_rate` に移行済み（CR-FASTAPI-009）
- `/api/projects/<project_id>/badge/last_run` (GET) - FastAPI `/api/projects/{id}/badge/last_run` に移行済み（CR-FASTAPI-009）
...
### Phase 3: CR-FASTAPI-010（予定）

**削除対象**:
- `src/nexuscore/webapp/api_external.py` の以下のエンドポイント:
  - `/api/v1/projects` (GET) - FastAPI に移行済み
  - `/api/v1/projects/<project_id>/run` (POST) - FastAPI に移行済み（CR-FASTAPI-009）
  - `/api/v1/projects/<project_id>/runs/latest` (GET) - FastAPI に移行済み（CR-FASTAPI-009）
- `src/nexuscore/webapp/api_badges.py` の以下のエンドポイント:
  - `/api/projects/<project_id>/badge/success_rate` (GET) - FastAPI に移行済み（CR-FASTAPI-009）
  - `/api/projects/<project_id>/badge/last_run` (GET) - FastAPI に移行済み（CR-FASTAPI-009）
