...
**変更内容**:
- ErrorResponse の docstring に「エラーコードカタログ.md に定義されたコードのみを使う」旨を追記
...
**実装内容**:
- エラーコードカタログ.md からエラーコードと HTTP ステータスの対応をパースする関数を実装
- OpenAPI スキーマから ErrorResponse を返すレスポンス定義を抽出する関数を実装
- カタログと OpenAPI の整合性をチェックするテストを実装

**テストケース**:
- `test_error_code_catalog_exists`: エラーコードカタログ.md が存在することを確認
- `test_error_code_catalog_parsable`: エラーコードカタログ.md が正しくパースできることを確認
...
- 「エラー仕様の単一ソースは エラーコードカタログ.md」という一文を明記
- エラーコード一覧のセクションにカタログへの参照を追加

**変更ファイル**: `README.md`

**変更内容**:
- API 構成セクションに「エラーコード: すべてのエラーコードは `docs/api/エラーコードカタログ.md` に定義されています（単一のソース）」を追加
...
- `docs/api/エラーコードカタログ.md` - Error Code Catalog（エラーコードの単一のソース）
- `tests/api/test_error_code_catalog.py` - カタログと OpenAPI の整合性チェックテスト
- `docs/api/CR-FASTAPI-015_完了報告.md` - 完了レポート（本ファイル）
...
- **カタログのパース**: エラーコードカタログ.md のパースは Markdown テーブルの形式に依存しています。テーブル形式を変更する場合は、テストも更新する必要があります

### 移行時の注意点

- 新しいエラーコードを追加する場合は、必ず エラーコードカタログ.md にも追記してください
...
- [エラーコードカタログ.md](./エラーコードカタログ.md) - Error Code Catalog（エラーコードの単一のソース）
- [CR-FASTAPI-006 Completion Report](./CR-FASTAPI-006_完了報告.md) - エラー標準化
- [CR-FASTAPI-014 Completion Report](./CR-FASTAPI-014_完了報告.md) - 認証エラー正規化
