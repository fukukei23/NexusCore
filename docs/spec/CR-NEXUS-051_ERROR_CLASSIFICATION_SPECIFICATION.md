...
**バージョン**: 1.1
**最終更新**: 2025-01-06
**ステータス**: 改訂中（Phase 2.5 再 Review 待ち）
...
**意味論レベルの定義**:
- **指数バックオフ（Exponential Backoff）**: リトライ回数が増えるにつれて、待機時間を指数関数的に増加させる戦略
- **固定間隔（Fixed Interval）**: すべてのリトライで同一の待機時間を使用する戦略
- **即座リトライ（Immediate Retry）**: 待機時間なしで即座に再試行する戦略（初回のみ）

**エラー種別ごとの推奨戦略**:
- `rate_limit`: 指数バックオフ（長め） - レートリミット解除を待つ必要があるため
- `timeout`: 指数バックオフ（中） - サーバー負荷が解消されるまでの待機
- `connection`: 指数バックオフ（短め） - ネットワーク接続の復旧を待つ
- `invalid_output`: 即座リトライ（初回のみ） - プロンプト調整後の即座再試行
...
**Step 2: エラーログの記録**
- 分類不能エラーは、`logger.warning()` レベルでログに記録する
- ログには、元のエラー情報、分類試行時のコンテキスト、分類失敗の理由を含める

**Step 3: 上位へのエラー伝播**
- 分類不能エラーは、`UnexpectedSystemError` として上位に伝播する
