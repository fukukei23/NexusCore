...
**参照（STIT / Phase 2.5）**: 仕様・テスト・実装の順序と Gate は [GOVERNANCE/SPEC_TEST_DRIVEN_ITERATION.md](../GOVERNANCE/SPEC_TEST_DRIVEN_ITERATION.md)（STIT）に従う。Phase 2.5（IRG）は [docs/ARCHITECTURE.md](ARCHITECTURE.md) および [GOVERNANCE/templates/REVIEW_PACKET_PHASE25_TEMPLATE.md](../GOVERNANCE/templates/REVIEW_PACKET_PHASE25_TEMPLATE.md) を参照。
...
- **API / FastAPI 系 CR の完了レポート**: `docs/api/`
  ...
  - 例: `docs/api/CR-FASTAPI-021_COMPLETION_REPORT.md`
...
- **レビュー結果**: Phase 2.5（IRG）を実施した場合は Verdict と指摘の要約
...
3. Phase 2.5（IRG）を実施する場合は、Review Packet のチェックリスト「Tests cover the Decision Table」を満たすようにテストを用意・実行する。
...
| 種類 | フォルダ | 用途 |
|------|----------|------|
| 決定履歴 | `DECISION_LOGS/` | Phase 2.5 レビュー結果・重要な判断の記録（append-only） |
| レビューパケット | `GOVERNANCE/review_packets/` | IRG のレビュー結果（RP-NEXUS-XXX_*.md） |
...
- **重要な決定・レビュー結果** → `DECISION_LOGS/`、`GOVERNANCE/review_packets/`
