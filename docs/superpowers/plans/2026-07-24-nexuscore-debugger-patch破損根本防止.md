...
def test_debug_history_accumulates(orchestrator_with_mock_debugger, tmp_path):
    """各試行が debug_history に記録される（スキーマ: attempt/ast/passed/err/result）。"""
    # 2ループ回して context.debug_history の長さ・キーを検証
```
...
```python
        ...
        try:                                       # 例外安全: sandbox/LLM API 例外でもロールバック保証
            while (
                ...
                ok, err = validate_python_syntax(str(fixed_code))
                if not ok:
                    ast_fail_streak += 1
                    error_log = (
            ...
            if not passed and primary_impl_path and impl_abs_path is not None:
                impl_abs_path.parent.mkdir(parents=True, exist_ok=True)
...
```python
    ...
    def _format_debug_history(history: list) -> str:
        """debug_history を人間可読の要約に整形。"""
        if not history:
            return "（デバッグ試行なし）"
        lines = []
        for h in history:
            attempt = h.get("attempt", "?")
            if h.get("result") == "no_fixed_code":
                lines.append(f"  attempt {attempt}: fixed_code 生成なし")
            elif h.get("ast") == "NG":
                lines.append(f"  attempt {attempt}: 構文NG({h.get('err', '')})")
            else:
                lines.append(f"  attempt {attempt}: 構文OK・pytest={'通過' if h.get('passed') else '失敗'}")
        return "\n".join(lines)
```
...
**3. Type consistency**: `validate_python_syntax -> tuple[bool,str]`（Task1定義→Task4使用）✅ / `debug_history: list[dict]`（Task2追加→Task4/5使用）✅ / `AST_FAIL_LIMIT`（Task3定義→Task4使用）✅ / `_clean_pytest_cache(project_path: str)`（Task3定義→Task4使用）✅
