...
- [ ] **Step 0: run_full_project の呼び出し元を確認（戻り値変更の安全確認）**

Run: `grep -rn "run_full_project(" src/ tests/ main_cli.py | grep -v "def run_full_project"`

各呼び出し元が戻り値を無視している（代入していない・チェーンしていない）ことを確認する。戻り値を使っている箇所があれば `OrchestratorContext | None` を受ける前提で更新する。
