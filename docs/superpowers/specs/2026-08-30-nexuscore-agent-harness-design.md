...
- **4ハードリミット**: ステップ25回/実行10分/tool呼出40回/**トークン200k**（発火**前**チェック・80%で通知）
  - ⚠️ **2026-09-19改訂**: 当初「トークン500k」だったが、これは下記のメッセージ履歴圧縮が無い前提の値であり、**同じspecが定める「ステップ25回」と両立していなかった**（実測: 圧縮なしだと送信量がステップ数の二次で増え〔k≈1,206 tokens/steps²〕、25ステップに**753,750トークン**必要）。圧縮の導入で25ステップ完走が約172,000トークンに収まるため200kへ改訂。実装は `Limits.max_tokens` を唯一の正典とし、CLI/ラッパー側はこれを参照する（宣言側と実効側が別の数字を持つ二重管理の禁止）
  - **整合の機械検査**: 「1step平均トークン × max_steps < max_tokens × abort_at_fraction」が成り立つこと。`test_max_steps_is_reachable_within_token_limit` が固定する。リミット値を変更する際はこのテストを必ず通すこと
- **メッセージ履歴の圧縮**（2026-09-19追加・MVP層）: ループは毎ステップ `msgs` 全体を再送するため、履歴を放置すると累計送信量がステップ数の**二次**で増える。直近 `keep_recent_tool_results`（既定3）件を超えた古い tool_result は**本文を1行インデックスへ畳む**
  - **残すもの**: tool名・引数・元の本文長（`[omitted: read_file({"path":"x.py"}) → 4,200 chars]`）。本文を丸ごと捨てるとLLMが同じ道具を再実行してステップがかえって増えるため、「もう実行した」と判る情報は残す
  - **消さないもの**: tool_result メッセージ自体は削除しない（assistantの `tool_calls` と1対1で対応する必要があるOpenAI契約の維持）
  - **冪等**: 毎ステップ呼ばれるため、既に畳んだものを再処理しないこと
  - 実測効果: 20ステップ時点の累計送信量が 1,562,605 → 482,249 文字（3.2倍削減）、増加次数が二次（R²=0.9994）から線形（R²=0.9697）へ転換
