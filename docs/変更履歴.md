...
## [2026-08-16] - C6 Step3: mypy CI 導入（残17エラー解消）で C6 完走
...
- `mypy.ini`: `python_version 3.11→3.12` 是正（pyproject requires-python>=3.12）・`api/archive` を exclude 追加（非現用コード・bandit/ruff と同じ除外方針）・未使用セクション（プリフィックス無し github_webhook）除去
