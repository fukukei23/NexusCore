...
### 未了（本修正のスコープ外）
- **トークン爆発の要因分離は未実施**。お題混入は一因だが、読む系お題のみのrunでも
  43〜48万トークンに達している（09-14/09-15/09-16）。上限結線により今後は180,000でabortするが、
  「なぜ読む系タスクで数十万トークンを要するか」は別途調査が必要
- G-2（ask応答時間の計装）・G-3（ask疑似TTYフィーダのラッパー化）・ask履歴べき等記録（store結線）
