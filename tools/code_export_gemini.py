# ======================================================================
# code_export_gemini.py — Gemini 10ファイル制限最適化（3ファイル構成）
# 外部共有“安全寄り”デフォルト（KEEP優先 / .envは除外）
# 2025-08-09 rev: safe-share-default + allowed_paths対応
# ======================================================================
...
# 最優先で含める（除外判定より優先）— Safe Share 最小安全セット
KEEP_FILES: Set[str] = {
    "requirements.txt",
    "pyproject.toml",
...
MAX_COMBINED_CODE_SIZE_MB = 10  # Gemini向け統合コード上限目安
WIN_PATH_CAP = 250
...
def path_is_ignored(path: Path, root: Path, patterns: Set[str], export_root: Path) -> bool:
    ...
    # 固定の除外ファイル
    if path.is_file() and path.name in IGNORED_FILES_USER:
        return True
...
def degree_centrality(mapping: Dict[Path, Set[Path]]) -> Dict[Path, float]:
    if not nx:
        return {k: float(len(v)) for k, v in mapping.items()}
    g = nx.DiGraph()
...
def collect_files(root: Path, max_single_mb: int, progress: gr.Progress) -> List[Path]:
    ...
    for i, p in enumerate(entries):
        ...
        if not include_by_keep:
            if path_is_ignored(p, root, patterns, export_root):
                pass
            else:
                if p.is_file() and (p.suffix.lower() in FILE_TYPES) and (p.stat().st_size <= cap_single):
                    if os.name == "nt":
                        try:
                            potential_path = export_root / p.relative_to(root)
                            if len(str(potential_path)) > WIN_PATH_CAP:
                                p = None
                        except ValueError:
                            p = None
                    if p is not None:
...
## 概要
このZIPは、`{project_prefix}` の重要ファイルを抽出し、Gemini解析に最適化したものです。
...
def export_for_gemini(roots: List[Path], max_single_mb: int, progress: gr.Progress) -> Path:
    ...
    session_dir = export_dir / f"{project_prefix}_gemini_{ts}"
    ...
    if not final_files:
        raise RuntimeError("処理対象ファイルがありません")

    tree_txt, path_list_txt = make_tree_and_list(final_files)

    progress(0.8, desc="Gemini用ファイル生成中...")
    project_info = generate_project_info_content(
        total_files, total_lines, stats, "Gemini", MAX_COMBINED_CODE_SIZE_MB,
        project_prefix, tree_txt=tree_txt, path_list_txt=path_list_txt, kept=list(KEEP_FILES)
    )

    combined_code = create_combined_code(final_files, progress)
...
def run_export_wrapper(roots_str: str, max_mb: int, progress=gr.Progress(track_tqdm=True)):
    ...
    if not roots_list:
        return None, "⚠️ 少なくとも1つのフォルダを選択してください"
    ...
    for path_str in roots_list:
        try:
            ...
            if not path.is_dir():
                return None, f"⚠️ フォルダではありません: {path_str}"
            roots.append(path)
        except Exception:
            return None, f"⚠️ 無効なパス: {path_str}"
...
📋 3ファイル:
1. PROJECT_INFO.md（統計/作成方法/ツリー/パス一覧）
2. COMBINED_CODE.py
3. README.md
🔐 KEEP: {", ".join(sorted(KEEP_FILES))}
"""
    except Exception as e:
        return f"⚠️ プレビュー生成エラー: {e}"
...
def create_interface():
    with gr.Blocks(title="Code Export for Gemini", css=".gr-button{min-width:6rem} .gr-textbox{font-family:monospace}", theme=gr.themes.Soft()) as demo:
        gr.Markdown("## 🤖 Gemini対応コードエクスポート（Safe Share デフォルト）")
        with gr.Row():
        ...
        gemini_preview = gr.Textbox(label="プレビュー", lines=8, interactive=False, visible=False)
...
def _get_allowed_paths() -> list[str]:
    """
    優先順位:
    1) 環境変数 GRADIO_ALLOWED_PATHS があればそれを採用（カンマ区切り）
    2) 既定の2ディレクトリを許可
    """
...
if __name__ == "__main__":
    print("🤖 Code Export for Gemini (Safe Share) 起動")

    allowed_paths = _get_allowed_paths()
    # 必要に応じて blocked_paths も追加可能。例:
    # blocked_paths = [r"C:\Users\USER\secrets"]  # allowedより優先
    blocked_paths = None

    # 127.0.0.1:7860 で起動。allowed_pathsを明示指定。
    ...
    if blocked_paths:
        launch_kwargs["blocked_paths"] = blocked_paths
