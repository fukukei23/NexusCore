...
NexusCore では、変更内容に応じて **Gate（制約レベル）** が自動的に適用されます。

- **Fast Gate**: ドキュメント修正・軽微な改善（最速）
- **Standard Gate**: 機能追加・通常の開発
- **Strict Gate**: エラー処理 / 再試行 / 自律挙動 / 外部API / セキュリティ

⚠️ **再試行・失敗時の最終挙動・外部API・自律修復に触れる変更は Strict Gate となり、  
仕様確認と独立レビューが必須です。**

通常の利用や軽微な変更は Fast Gate のままで問題ありません。  
詳細は `docs/ARCHITECTURE.md` および `GOVERNANCE/` を参照してください。
...
NexusCore は **STIT+IRG** (Spec & Test Driven Iteration + Independent Review Gate) で運用します。

- **[Architecture](docs/ARCHITECTURE.md)** - SSOT入口
- **[Governance](GOVERNANCE/README.md)** - ガバナンス資産
- **[Project Profile](PROJECT_PROFILES/PROJECT_PROFILE_NEXUSCORE.md)** - 制約宣言
- **[Decision Log](DECISION_LOGS/DECISION_LOG.md)** - 意思決定記録
