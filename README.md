...
### 🎯 Authority Runner（実行制御）
- **段階的実行制御**: 権限レベル（HUMAN_CONTROLLED / PARTIALLY_AUTONOMOUS / FULLY_AUTONOMOUS）に応じた自動実行範囲の制御
- **Pause/Resume機能**: フェーズ境界での一時停止と任意のタイミングでの再開
- **Run State管理**: 実行状態の永続化、整合性検証、スキーマバリデーション
- **Run Lock**: ファイルベースの並行実行制御（複数プロセス間での同一run_id実行の防止）
- **Run View API**: 実行状態の可視化とモニタリング用RESTエンドポイント

### 🏛️ Governance Tools（ガバナンス自動化）
- **CR（Change Request）管理**: 変更仕様書の作成・検証・追跡を自動化
- **品質ゲート**: テストカバレッジ、コード品質、ドキュメント整合性の自動検証
- **Spec-driven開発**: 仕様書ベースの開発フロー（`docs/spec/` 配下でCR管理）
- **CI依存性ガード**: `requirements.txt` と `requirements-ci-safe.lock` の不整合検出
- **完了レポート自動生成**: CR完了時の実装内容を標準形式で記録
...
│
├── docs/                    # ドキュメント
│   ├── governance/          # 統治ルール・ガバナンス
│   │   ├── NEXUSCORE_GOVERNANCE.md          # プロジェクト統治ルール
│   │   ├── AI_COLLABORATION_RULES.md        # 人間×AI協働プロトコル
│   │   └── CR_SPEC_FINGERPRINT.txt          # CR仕様の指紋管理
│   │
│   ├── overview/            # プロジェクト概要
│   │   ├── 00_OVERVIEW_INDEX.md             # ドキュメント体系インデックス
│   │   ├── 01_Project_Vision_and_Strategy.md # ビジョンとビジネス戦略
│   │   ├── 02_Technical_Architecture.md     # 技術アーキテクチャ
│   │   ├── 03_Development_Roadmap.md        # 開発ロードマップ
│   │   └── 04_Developer_Internal_Guide.md   # 開発者向け環境構築・運用ガイド
│   │
│   ├── spec/                # 技術仕様書（CR-NEXUS-XXX）
│   │   ├── SPEC_TEMPLATE.md                 # CR作成テンプレート
│   │   ├── CR-NEXUS-012_Authority_Level_Control.md  # Authority Level仕様
│   │   └── CR-NEXUS-016~024_RunState契約群.md       # Run State関連契約
│   │
│   ├── api/                 # API仕様・完了レポート
│   │   ├── README.md                        # API仕様インデックス
│   │   ├── PROJECTS_API_CONTRACT.md         # Projects API契約
│   │   └── CR-NEXUS-XXX_COMPLETION_REPORT.md # CR完了レポート群
│   │
│   ├── srs/                 # 要求仕様運用ルール
│   │   └── README.md                        # SRS運用ルール
│   │
│   ├── CI_SAFE_LOCK.md      # CI依存関係ロック管理ガイド
│   ├── CI_TEST_STRATEGY.md  # CIテスト戦略（Safe/Full分離）
│   ├── ARCHITECTURE.md      # システムアーキテクチャドキュメント
│   └── reports/             # テストレポート
│
├── dev_tools/               # 開発支援ツール
├── tools/                   # 補助ユーティリティ
│   ├── scaffold_cr.py       # CR作成自動化（440行）
│   └── update_ci_safe_lock.py # CI依存性ガード（184行）
│
└── output/                  # 実行ログ・一時ファイル
```
...
### コアドキュメント
- **[プロジェクト概要インデックス](docs/overview/00_OVERVIEW_INDEX.md)** - ドキュメント体系の全体像
- **[ガバナンスルール](docs/governance/NEXUSCORE_GOVERNANCE.md)** - プロジェクト統治ルール、権限レベル定義
- **[技術アーキテクチャ](docs/overview/02_Technical_Architecture.md)** - NexusOSモデル、エージェント構成
- **[開発者ガイド](docs/overview/04_Developer_Internal_Guide.md)** - 開発環境構築・運用ガイド

### 仕様書・レポート
- **[Specテンプレート](docs/spec/SPEC_TEMPLATE.md)** - CR作成テンプレート
- **[API仕様インデックス](docs/api/README.md)** - API仕様、エラーコードカタログ
- **[完了レポート](docs/api/)** - 各CR完了レポート（CR-NEXUS-XXX_COMPLETION_REPORT.md）

### テスト・品質
- **[カバレッジレポート](docs/reports/COVERAGE_SUMMARY.md)** - テストカバレッジの詳細
- **[HTMLカバレッジレポート](docs/reports/coverage/index.html)** - 対話的カバレッジレポート
- **[CIテスト戦略](docs/CI_TEST_STRATEGY.md)** - Safe Tests / Full Tests分離
...
NexusCore は包括的なドキュメント体系を持っています。`docs/` ディレクトリに整理されたドキュメント群：

### ガバナンス・統治（`docs/governance/`）
- **[NEXUSCORE_GOVERNANCE.md](docs/governance/NEXUSCORE_GOVERNANCE.md)** - プロジェクト全体の統治ルール、権限レベル定義、凍結方針
- **[AI_COLLABORATION_RULES.md](docs/governance/AI_COLLABORATION_RULES.md)** - 人間×AI協働プロトコル、提案ベースフロー
- **CR_SPEC_FINGERPRINT.txt** - CR仕様の指紋管理

### プロジェクト概要（`docs/overview/`）
- **[00_OVERVIEW_INDEX.md](docs/overview/00_OVERVIEW_INDEX.md)** - ドキュメント体系の全体像とナビゲーション
- **[01_Project_Vision_and_Strategy.md](docs/overview/01_Project_Vision_and_Strategy.md)** - ビジョン、ビジネス戦略、ターゲット市場
- **[02_Technical_Architecture.md](docs/overview/02_Technical_Architecture.md)** - NexusOSモデル、エージェント構成、自己修復メカニズム
- **[03_Development_Roadmap.md](docs/overview/03_Development_Roadmap.md)** - 開発ロードマップとマイルストーン
- **[04_Developer_Internal_Guide.md](docs/overview/04_Developer_Internal_Guide.md)** - 開発環境構築・運用ガイド

### 技術仕様書（`docs/spec/`）
**Spec-driven開発**: 新機能・変更はCR（Change Request）仕様書ベースで管理

- **[SPEC_TEMPLATE.md](docs/spec/SPEC_TEMPLATE.md)** - CR作成テンプレート
- **[CR-NEXUS-012_Authority_Level_Control.md](docs/spec/CR-NEXUS-012_Authority_Level_Control.md)** - Authority Level仕様
- **CR-NEXUS-016〜024** - RunState UX、Resume再構築モデル、JSONスキーマ、I/O契約、整合性、並行実行、FSロック

### API仕様・完了レポート（`docs/api/`）
- **[README.md](docs/api/README.md)** - API仕様インデックス、エラーコードカタログ参照
- **[PROJECTS_API_CONTRACT.md](docs/api/PROJECTS_API_CONTRACT.md)** - Projects API契約仕様
- **CR-NEXUS-XXX_COMPLETION_REPORT.md** - 各CR完了レポート（実装内容、変更ファイル、テスト結果）

### 運用ドキュメント（`docs/`）
- **[CI_SAFE_LOCK.md](docs/CI_SAFE_LOCK.md)** - CI依存関係ロック管理ガイド（`tools/update_ci_safe_lock.py` 対応）
- **[CI_TEST_STRATEGY.md](docs/CI_TEST_STRATEGY.md)** - CIテスト戦略（Safe Tests / Full Tests分離）
- **[srs/README.md](docs/srs/README.md)** - SRS（Software Requirements Specification）運用ルール
...
**役割別の導線:**
- **新規開発者向け**: [開発ガイド（軽量版）](DEVELOPMENT.md) → [docs/overview/04_Developer_Internal_Guide.md](docs/overview/04_Developer_Internal_Guide.md) → [README_VENV.md](README_VENV.md)
- **アーキテクチャ理解**: [docs/overview/02_Technical_Architecture.md](docs/overview/02_Technical_Architecture.md) → [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)
- **仕様書作成**: [docs/spec/SPEC_TEMPLATE.md](docs/spec/SPEC_TEMPLATE.md) → [docs/srs/README.md](docs/srs/README.md)
...
NexusCoreでは、仕様書ベースの開発フロー（Spec-driven Development）を採用しています。

#### CR（Change Request）作成フロー

1. **仕様書作成**: `docs/spec/CR-NEXUS-XXX_<title>.md` を作成
   - テンプレート: [docs/spec/SPEC_TEMPLATE.md](docs/spec/SPEC_TEMPLATE.md)
   - SRSトレーサビリティを記載（関連する要求仕様へのリンク）
   - スコープ、設計方針、テスト方針、完了条件を明記

2. **実装**: 仕様書に従ってコード実装
   - `src/nexuscore/` 配下に実装
   - 既存アーキテクチャパターンに従う（BaseAgent継承、LLMRouter利用など）

3. **テスト**: テストケース追加・実行
   - `tests/` 配下にユニット・統合テストを追加
   - カバレッジ80%以上を目指す

4. **完了レポート**: `docs/api/CR-NEXUS-XXX_COMPLETION_REPORT.md` を作成
   - 実装日時、概要、ゴール、実装ステップ、変更ファイル一覧、動作確認結果を記載

5. **PR作成**: mainブランチへマージ
   - PRタイトル: `feat: <CR-ID> <title>`
   - 品質ゲート（Pylint, Mypy, テスト）をパス

#### ツール支援

- **CR作成自動化**: `tools/scaffold_cr.py` - CR仕様書・完了レポートのスキャフォールディング
- **CI依存性ガード**: `tools/update_ci_safe_lock.py` - requirements.txt/lock.txtの不整合検出
- **品質ゲート自動実行**: Guardian Agent - コード品質、テストカバレッジの自動検証

詳細は [docs/srs/README.md](docs/srs/README.md) と [docs/spec/SPEC_TEMPLATE.md](docs/spec/SPEC_TEMPLATE.md) を参照してください。
